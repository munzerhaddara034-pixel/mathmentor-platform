...
function exponentialSolution(language: LessonLanguage, track: CertificateTrack): MathSolution {
  return assembleSolution({
    question: "f(x)=(x-1)e^x. Find the domain, f'(x), and the minimum.",
    summary: "Terminale LS/GS/SE study of f(x)=(x-1)e^x — official exam pattern.",
    finalAnswer: "D_f=R, f'(x)=x e^x, minimum (0,-1)",
    finalAnswerLatex: "f'(x)=x e^{x},\\ \\min(0,-1)",
    steps: [
      {
        title: "Domain",
        titleFr: "Ensemble de définition",
        titleAr: "مجموعة التعريف",
        latex: "D_f=\\mathbb{R}",
        explanationEn: "The exponential is defined on R and x−1 is a polynomial, so the product is defined on R. No slogan: name both factors.",
        explanationFr: "L’exponentielle est définie sur R et x−1 est polynomiale : le produit l’est sur R.",
      ...
      {
        title: "Sign of f' and minimum",
        titleFr: "Signe de f' et minimum",
        titleAr: "إشارة المشتق والحد الأدنى",
        latex: "f'(x)=0\\iff x=0;\\quad f(0)=-1",
        explanationEn: "e^x>0 always, so f' has the sign of x. f decreases on (−∞,0] and increases on [0,+∞). The minimum is (0,−1).",
        explanationFr: "e^x>0, donc f' a le signe de x. Minimum en (0,−1).",
        explanationAr: "e^x موجبة دائماً فإشارة f' كإشارة x. الحد الأدنى (0,−1).",
      },
      {
        title: "Limits (justified)",
        titleFr: "Limites justifiées",
        titleAr: "نهايات معلّلة",
        latex: "\\lim_{x\\to+\\infty}f(x)=+\\infty,\\quad \\lim_{x\\to-\\infty}f(x)=0",
        explanationEn: "At +∞ the product (+∞)×(+∞). At −∞ rewrite with t=−x: −(t+1)e^{−t} → 0. Never write (−∞)×0 without rewriting.",
        explanationFr: "En +∞ : (+∞)×(+∞). En −∞ : t=−x, forme −(t+1)e^{−t} → 0. On ne laisse pas (−∞)×0.",
        explanationAr: "عند +∞ الناتج +∞. عند −∞ نعيد الكتابة ولا نترك شكل (−∞)×0.",
      },
    ],
    graph: {
      fn: "(x-1)*exp(x)",
    ...
    trap: {
      wrong: "Writing f'(x)=e^x, or treating (−∞)×0 as 0, or applying the power rule to e^x.",
      wrongFr: "Écrire f'(x)=e^x, ou traiter (−∞)×0 comme 0, ou appliquer nx^{n−1} à e^x.",
      correction: "Product rule + rewrite the limit at −∞. Box f'(x)=x e^x and min (0,−1).",
      correctionFr: "Produit + réécriture de la limite en −∞. Encadrer f'(x)=x e^x et min (0,−1).",
      latex: "f'(x)=x e^{x}",
    },
    ...
    steps: [
      {
        ...
        latex: "a^{2}+b^{2}=c^{2}",
        explanationEn: "Right angle required. Legs a,b; hypotenuse c opposite the right angle.",
...
export function demoSolve(request: SolveRequest): MathSolution {
  ...
  if (/complex|مركب|argand/.test(n)) {
    return assembleSolution({
      ...
      steps: [
        { title: "Definition", titleFr: "Définition", latex: "|z|=\\sqrt{a^{2}+b^{2}}", explanationEn: "For z=a+ib, |z| is the Euclidean length on the Argand plane.", explanationFr: "Pour z=a+ib, |z| est la longueur euclidienne." },
