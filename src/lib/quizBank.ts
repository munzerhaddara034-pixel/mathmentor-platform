...
export function questionsForLesson(lessonId: string, custom: QuizQuestion[] = []): QuizQuestion[] {
  ...
  const skipTemplates = fromExtra.length >= TEMPLATE_SKIP_THRESHOLD;
  const generated: QuizQuestion[] =
    !skipTemplates && lesson
      ? [
...
/** Exam packs: ~30% routine, ~50% standard, ~20% harder, without filler templates. */
export function sampleExamQuestions(pool: QuizQuestion[], limit: number): QuizQuestion[] {
  if (pool.length <= limit) return pool;
  const easy = shuffle(pool.filter((item) => item.difficulty === 1));
  const standard = shuffle(pool.filter((item) => item.difficulty === 2 || item.difficulty === 3));
  const hard = shuffle(pool.filter((item) => item.difficulty === 4));
  const nEasy = Math.max(1, Math.round(limit * 0.3));
  const nHard = Math.max(1, Math.round(limit * 0.2));
  const nStandard = Math.max(1, limit - nEasy - nHard);
  const picked = [...easy.slice(0, nEasy), ...standard.slice(0, nStandard), ...hard.slice(0, nHard)];
  const leftover = shuffle(pool.filter((item) => !picked.includes(item)));
  while (picked.length < limit && leftover.length) picked.push(leftover.shift()!);
