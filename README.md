...
| بنك **37** سؤالاً للنهايات (شريحة أولى داخل بنك الدوال) | `/practice/take?lessonId=grade-12-ch1` |
| مسابقة **علوم الحياة — الدوال** (66 سؤالاً في الملف، المسابقة 16 مرتّبة سهل→صعب) | `/practice` أو `/practice/take?bank=g12-ls-functions&mode=contest` |
| نصوص منطوقة لثلاثة دروس (للتسجيل الحي لاحقاً) | `content/grade-12-ls-limits/` |

التدريب الحر لدرس النهايات يعرض شريحة النهايات. امتحان الدرس يسحب 12 سؤالاً بمزيج صعوبة. مسابقة الدوال تستخدم بنك الموضوع مرتّباً سهل→صعب.
...
```
content/banks/g12-ls/functions.json   ← تجريبي (هذا الإصدار)
content/banks/g12-ls/vectors.json     ← التالي
content/banks/g12-ls/integration.json
content/banks/g12-se/…
content/banks/g12-gs/…
```

بنك **الدوال** يضم ثلاث شرائح: النهايات (مع الفيديو) ثم الاستمرار ثم المشتقات. الملف مرتّب سهل → متوسط → صعب. كل بند يحمل `source.models` (مثل `LS all sessions.pdf`) و`source.kind`: `generated-in-official-style` أو لاحقاً `session-extract`.

كيف يُضاف الموضوع التالي: انسخ الملف، غيّر `id` والعناوين والشرائح، املأ الأسئلة، ثم سجّله في `src/lib/topicBanks.ts`. التفاصيل في `content/banks/README.md`.

الطالب: `/practice` → «مسابقة الدوال» (16 سؤالاً / 25 دقيقة، سهل ثم متوسط ثم صعب) أو تدريب البنك كاملاً (66).
