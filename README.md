...
الطالب: `/exams` → فرع الشهادة × السنة/الدورة × الموضوع، أو `/practice` → مسابقة الفرع (LS / SE / GS / Brevet / LH).

---

## بنك الامتحانات ومحرّك المسابقة / Exam & quiz engine

المحرّك يوسّع مشغّل `/practice/take` الحالي (لا توجد ثلاث واجهات اختبار منفصلة). الكتالوج الجديد:

| ماذا | أين |
| --- | --- |
| تصفح حسب الفرع (Brevet، LS، GS، SE، LH) والسنة والدورة والموضوع | [`/exams`](http://localhost:3000/exams) |
| ورقة كاملة متعددة المسائل (أولاً / ثانياً / …) مع مؤقت وMathJax | `/exams/play?pack=paper-LS-2024-ordinary` |
| تدريب موضوع (دوال، أعداد مركبة، احتمالات، هندسة تحليلية، متتاليات، معادلات تفاضلية) | `/exams/play?pack=topic-GS-complex` |
| تصدير ورقة PDF للطباعة (ترويسة Math Mentor بأسلوب مبسّط — ليست وثيقة وزارة) | `/exams/print?pack=paper-LS-2024-ordinary&mode=paper` ثم **تحميل / طباعة PDF** واختر «حفظ كملف PDF» |
| الحلول + جدول الباريم بعد التسليم | نفس المشغّل بعد «تسليم الامتحان»، أو `mode=solutions` في رابط الطباعة |

وسم السنة/الدورة مثل `style:official-2024-ordinary` يعني **تدريب بأسلوب النماذج الرسمية** لتلك الدورة — **ليست** أوراقاً رسمية منسوخة.

الباريم: كل سؤال له علامات، والمجموع 20 للنموذج الكامل. بعد التسليم تظهر العلامة لكل مسألة وخطوات الحل.

البنوك القديمة في `content/banks/**` ما زالت المصدر؛ أُضيفت مسودات للمتتاليات وLH والمعادلات التفاضلية دون حذف الملفات القائمة.
...
| المسار | المحتوى |
| --- | --- |
| `/` | الصفحة الرئيسية (Math Mentor · الأستاذ منذر حدارة) |
| `/login` | تسجيل الدخول |
...
| `/practice` | تمارين واختبارات البنوك (نفس المحرّك) |
| `/exams` | كتالوج الامتحانات: فرع × سنة/دورة × موضوع |
| `/exams/play` | مشغّل الامتحان الموقوت (مسائل مجزّأة + MathJax) |
| `/exams/print` | ورقة قابلة للطباعة / حفظ PDF |
