...
النماذج الرسمية على جهاز منذر (`LS all sessions.pdf`, `SE_All Sessions.pdf`, `GS-All.pdf`) **لا تُنسخ** إلى Git. يُستخرج أسلوب السؤال، ثم يُوزَّع على ملفات موضوع. الفروع الأربعة (متوسط، علوم حياة، اجتماع واقتصاد، علوم عامة) مُدرجة في `/practice` عبر `content/banks/certificates.json`.
...
content/banks/g12-se/…                      ← اجتماع واقتصاد (لاحقاً)
content/banks/g12-gs/…                      ← علوم عامة (لاحقاً)
```
...
**الشهادة المتوسطة (صف 9):** البنوك الخمسة جاهزة: أعداد، جبر، مسائل لفظية، هندسة، هندسة تحليلية. **علوم الحياة / اجتماع واقتصاد / علوم عامة:** المنفَّذ من علوم الحياة هو دراسة الدوال؛ باقي بنوك الصف 12 يزرعها لاحقاً وكيل آخر. الفروع الأربعة ظاهرة على `/practice`.

كيف يُضاف موضوع جديد: ملف JSON في مجلد الفرع، `certificate` مطابق (Brevet / LS / SE / GS)، ثم استيراد في `src/lib/topicBanks.ts`. التفاصيل في `content/banks/README.md`.
